...
So what's the culprit? You remembered that the number of the app's users has increased a lot in the past few days, but the number of backend servers for the app hasn't changed for months. Maybe these servers cannot support that many users' requests? That's possible. But the processing time initiated by the "START" button isn't taken by the backend servers entirely. But before doing that, you first recorded the action in the notes:
...
<div class="callout">
    ...
    <p>Maybe it's because the backend servers are overloaded. I need to check its memory usage first.</p>
</div>
