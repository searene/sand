...
[Agenda](https://agenda.com/) is built based on the idea that notes and dates should be combined together, which is closely aligned with ours. Strictly speaking it's not a TODO app, at least not a traditional one, the official website describes the app as *Notes meets Calendar*. But since it can set a date for a block of notes, you can use it as a So it's natural to add notes for a TODO task in Agenda. What's more, Agenda stresses more on TODO notes than the TODO itself, you will notice that just by taking a look at its UI design: it's full of notes.
