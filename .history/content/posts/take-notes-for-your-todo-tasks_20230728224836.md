...
So what's the culprit? You remembered that the number of the app's users has increased a lot in the past few days, but the number of backend servers for the app hasn't changed for months. Maybe these servers cannot support that many users' requests? That's possible. But the processing time initiated by the "START" button isn't taken . But before doing that, you first recorded the action in the notes:
