...
So what's the culprit? You remembered that the number of the app's users has increased a lot in the past few days, but the number of backend servers for the app hasn't changed for months. Maybe these servers cannot support that many users' requests? That's possible. If that's the reason, the next thing you would need to do is to check these servers' status. But before doing that, you first recorded the action in the notes:
