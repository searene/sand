...
So the servers' request time has indeed increased, the next step would be to check the servers' various indicators, like CPU, memory, load, etc. You should also append these information to the notes, not only for guiding you through fixing the issue, but also for a potential reference in
